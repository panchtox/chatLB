...
$lista_archivos=scandir($chats_folder);
$chunk_size=5000;

for ($i=(sizeof($lista_archivos)-$chunk_size); $i < sizeof($lista_archivos) ; $i++) { 
	$chat_posts[str_replace(".txt", "", $lista_archivos[$i])]['content']=file_get_contents($chats_folder.$lista_archivos[$i]);
	$post_parts=preg_split("#\:#", $chat_posts[str_replace(".txt", "", $lista_archivos[$i])]['content']);
	$chat_posts[str_replace(".txt", "", $lista_archivos[$i])]['author']=$post_parts[0];
	$chat_posts[str_replace(".txt", "", $lista_archivos[$i])]['content']=preg_replace("/^[\w\W]+:/", "", $chat_posts[str_replace(".txt", "", $lista_archivos[$i])]['content']);
	$chat_posts[str_replace(".txt", "", $lista_archivos[$i])]['content']=trim($chat_posts[str_replace(".txt", "", $lista_archivos[$i])]['content']);
...
<html>
	...
	<body>
		...
		<script type="text/javascript">
			...
			var formatDateTime=d3.time.format("%y%m%d%H%M").parse;
			...
			var h=viewportHeight*0.30, w=viewportWidth*0.98, margin=30;
